...
def configure_other_settings():
    ...
    if not hasattr(settings, "ACCOUNT_ACTIVATION_DAYS"):
        warnings.warn("You should set 'ACCOUNT_ACTIVATION_DAYS' in settings " "module.")
        setattr(settings, "ACCOUNT_ACTIVATION_DAYS", 7)
    if not hasattr(settings, "_REGISTRATION_ADMIN_REQ_ATTR_NAME_IN_MODEL_INS"):
        setattr(
            settings,
    ...
    if not hasattr(settings, "REGISTRATION_REGISTRATION_EMAIL"):
        setattr(settings, "REGISTRATION_REGISTRATION_EMAIL", True)
