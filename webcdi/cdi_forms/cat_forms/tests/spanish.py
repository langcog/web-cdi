...
class CATSpanishAdministrationDataItemTest(TestCase):
    ...
    def sequence_test(self, file_name):
        ...
        for row in contents[1:]:
            self.assertEqual(response.status_code, 200)
            self.assertIsInstance(response.context['form'], CatItemForm)
            self.assertContains(response, f"¿Su hijo/a dice ...{row[col_names.index('item')]} ?")
            payload = {
    ...
    def test_spanish_start_values(self):
        self.start_values(f'{settings.BASE_DIR}/cdi_forms/cat_forms/tests/test_data/spanish_start.csv')

    def test_spanish_seq_1(self):
        self.sequence_test(f'{settings.BASE_DIR}/cdi_forms/cat_forms/tests/test_data/spanish_sequence_1.csv')

    def test_spanish_seq_2(self):
        self.backgroundinfo.age=24
        self.backgroundinfo.save()
        self.sequence_test(f'{settings.BASE_DIR}/cdi_forms/cat_forms/tests/test_data/spanish_sequence_2.csv')
