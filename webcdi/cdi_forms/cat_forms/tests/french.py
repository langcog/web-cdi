...
class CATFrenchAdministrationDataItemTest(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse("cat_forms:administer_cat_form", kwargs={'hash_id': self.administration.url_hash})
    ...
    def start_values(self, file_name):
        for age in range(12, 37):
            ...
            self.assertContains(response, f"Votre enfant comprend-il et dit-il ... {word} ?")
    ...
    def sequence_test(self, file_name):
        ...
        for row in contents[1:]:
            self.assertEqual(response.status_code, 200)
            self.assertIsInstance(response.context['form'], CatItemForm)
            self.assertContains(response, f"Votre enfant comprend-il et dit-il ... {row[col_names.index('item')]} ?")
            payload = {
    ...
    def test_french_start_values(self):
        self.start_values(f'{settings.BASE_DIR}/cdi_forms/cat_forms/tests/test_data/french_start.csv')

    def test_french_seq_1(self):
        self.sequence_test(f'{settings.BASE_DIR}/cdi_forms/cat_forms/tests/test_data/french_sequence_1.csv')

    def test_french_seq_2(self):
        self.backgroundinfo.age=24
        self.backgroundinfo.save()
        self.sequence_test(f'{settings.BASE_DIR}/cdi_forms/cat_forms/tests/test_data/french_sequence_2.csv')
