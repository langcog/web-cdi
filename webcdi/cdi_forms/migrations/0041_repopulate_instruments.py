...
def populate_items(apps, schema_editor):
    ...
    for curr_instrument in input_instruments:
        ...
        try:
            instrument_model = apps.get_model(
                app_label="cdi_forms", model_name=instrument_obj.name
            )
        except:
            break
        ...
        for row in range(1, nrows):
            ...
            if len(row_values) > 1:
                ...
                if item_type not in ["combination_examples"]:
                    try:
                        if Choices:
                            choices_key = Choices.objects.get(
                                choice_set_en=item_choices
                            )
                        else:
                            choices_key = item_choices
                    except:
                        raise IOError(
                            "Can't find choice set %s in model for %s (%s)"
                            % (
                                item_category,
                                itemID,
                                item_choices,
                ...
                cdi_item, created = instrument_model.objects.update_or_create(
                    itemID=itemID,
                    defaults=data_dict,
                )
...
def fixItemOrders(apps, schema_editor):
    ...
    administration_data.objects.filter(
        administration__study__instrument__name="English_WS", item_ID="item_292"
    ).update(item_ID="item_XXX")
    administration_data.objects.filter(
        administration__study__instrument__name="English_WS", item_ID="item_293"
    ).update(item_ID="item_292")
    administration_data.objects.filter(
        administration__study__instrument__name="English_WS", item_ID="item_XXX"
    ).update(item_ID="item_293")

    administration_data.objects.filter(
        administration__study__instrument__name="English_WS", item_ID="item_282"
    ).update(item_ID="item_YYY")
    administration_data.objects.filter(
        administration__study__instrument__name="English_WS", item_ID="item_283"
    ).update(item_ID="item_282")
    administration_data.objects.filter(
        administration__study__instrument__name="English_WS", item_ID="item_YYY"
    ).update(item_ID="item_283")

    administration_data.objects.filter(
        administration__study__instrument__name="English_WS", item_ID="item_131"
    ).update(item_ID="item_ZZZ")
    administration_data.objects.filter(
        administration__study__instrument__name="English_WS", item_ID="item_132"
    ).update(item_ID="item_131")
    administration_data.objects.filter(
        administration__study__instrument__name="English_WS", item_ID="item_ZZZ"
    ).update(item_ID="item_132")
