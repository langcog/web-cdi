...
def max_subject_id(study_obj):
    if study_obj.study_group:
        ...
        max_id = Administration.objects.filter(study__in=related_studies).aggregate(
            Max("subject_id")
        )["subject_id__max"]
    else:
...
def max_repeat_num(administration):
    max_id = Administration.objects.filter(
        study=administration.study, subject_id=administration.subject_id
    ).aggregate(Max("repeat_num"))[
        "repeat_num__max"
    ]  # Find the subject ID in this study with the highest number
