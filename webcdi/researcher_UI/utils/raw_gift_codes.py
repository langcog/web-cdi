...
def get_gift_card_regex(gift_type):
    ...
    if gift_type == "Tango":
        return re.compile(
            r"^(?:www.rewardlink.io|www.rl.tangocard.com)/r/1/[A-Za-z0-9-_]{43}$"
        )
    return re.compile(r"^[A-Za-z0-9-]+$")


def raw_gift_code_fun(
    request, gift_type, raw_gift_amount, study_obj, new_study_name, raw_gift_codes
):
    amount_regex = None
    ...
    if raw_gift_codes:
        ...
        if len(invalid_gift_codes) > 0:
            messages.error(
                request,
                mark_safe(f"The following codes are invalid: {invalid_gift_codes}."),
            )
        ...
        if amount_regex:
            ...
            if len(used_codes) > 0:
                messages.warning(
                    request,
                    mark_safe(f"The following codes are previously used: {used_codes}"),
                )
            if len(new_codes) > 0:
                messages.success(
                    request,
                    mark_safe(f"The following codes have been added: {new_codes}"),
                )
        else:
            messages.error(
                request,
                mark_safe(f"The amount {raw_gift_amount} is invalid"),
            )
