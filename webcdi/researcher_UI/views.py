...
def download_data(request, study_obj, administrations=None):  # Download study data
    ...
    model_header = get_model_header(
        study_obj.instrument.name
    )  # Fetch the associated instrument model's variables
...
def download_dictionary(
    request, study_obj
):  # Download dictionary for instrument, lists relevant information for each item
...
@login_required
def download_links(
    request, study_obj, administrations=None
...
@login_required
def console(
    request, study_name=None, num_per_page=20
):  # Main giant function that manages the interface page
    ...
    if (
        request.method == "POST"
    ):  # If submitting data, make sure that study is allowed to be edited by current user
        data = {}
        ...
        if permitted:  # If user is permitted to update study
            ...
            ids = request.POST.getlist(
                "select_col"
            )  # Get the list of administrations with a clicked checkbox
            if all(
                [x.isdigit() for x in ids]
            ):  # Check that the administration numbers are all numeric
                if (
                    "administer-selected" in request.POST
                ):  # If the 'Re-administer Participants' button was clicked
                    num_ids = map(int, ids)  # Force numeric IDs into a list of integers
                    new_administrations = []
                    sids_created = (
                        set()
                    )  # This is used to ensure we don't add the same subject twice if it is ticked twice with different repeat_nums

                    for nid in num_ids:  # For each ID number
                        admin_instance = administration.objects.get(
                            id=nid
                        )  # Grab the associated administration object
                        sid = (
                            admin_instance.subject_id
                        )  # Pull subject ID from the administration object (unique to each study but not that whole database)
                        if sid in sids_created:
                            continue
                        old_rep = administration.objects.filter(
                            study=study_obj, subject_id=sid
                        ).count()  # Count the number of administrations previously given to this subject within this study
                        ...
                        sids_created.add(
                            sid
                        )  # Add new administration to the set of to-be-added administrations

                    administration.objects.bulk_create(
                        new_administrations
                    )  # Add new administrations to administration model en masse
                    refresh = True  # Refresh page to reflect table changes

                elif (
                    "delete-selected" in request.POST
                ):  # If 'Delete Selected Data' was clicked
                    num_ids = list(
                        set(map(int, ids))
                    )  # Force ID #s into a list of integers
                    administration.objects.filter(
                        id__in=num_ids
                    ).delete()  # Delete administrations with IDs found in the list of to-be-deleted IDs
                    refresh = True  # Refresh page to reflect table changes

                elif (
                    "download-links" in request.POST
                ):  # If 'Download Selected Data (Links)' was clicked
                    administrations = []
                    num_ids = list(
                        set(map(int, ids))
                    )  # Force IDs into a list of integers
                    administrations = administration.objects.filter(
                        id__in=ids
                    )  # Grab a queryset of administration objects with administration IDs found in list
                    return download_links(
                        request, study_obj, administrations
                    )  # Send queryset to download_links function to return a CSV of subject data
                    refresh = True  # Refresh page to reflect table changes

                elif (
                    "download-selected" in request.POST
                    ...
                    if study_obj.instrument.form in settings.CAT_FORMS:
                        ...
                        return download_data(
                            request, study_obj, administrations
                        )  # Send queryset to download_data function to return a CSV of subject data
                    refresh = True  # Refresh page to reflect table changes

                elif (
                    "download-selected-summary" in request.POST
                ):  # If 'Download Selected Data' was clicked
                    num_ids = list(
                        set(map(int, ids))
                    )  # Force IDs into a list of integers
                    ...
                    "delete-study" in request.POST
                ):  # If 'Delete Study' button is clicked
                    ...
                    "download-study-csv" in request.POST
                ):  # If 'Download Data' button is clicked
                    administrations = administration.objects.filter(
                        study=study_obj
                    )  # Grab a queryset of administration objects within study
                    if study_obj.instrument.form in settings.CAT_FORMS:
                        return download_cat_data(request, study_obj, administrations)
                    else:
                        return download_data(
                            request, study_obj, administrations
                        )  # Send queryset to download_data and receive a CSV of responses

                elif (
                    "download-summary-csv" in request.POST
                ):  # If 'Download Summary' button is clicked
                    administrations = administration.objects.filter(
                        study=study_obj
                    )  # Grab a queryset of administration objects within study
                    if study_obj.instrument.form in settings.CAT_FORMS:
                    ...
                    "download-study-scoring" in request.POST
                ):  # If 'Download Data' button is clicked
                    administrations = administration.objects.filter(
                        study=study_obj
                    )  # Grab a queryset of administration objects within study
                    ...
                    "download-study-scoring-selected" in request.POST
                ):  # If 'Download Data' button is clicked
                    ...
                    "download-dictionary" in request.POST
                ):  # If 'Download Dictionary Data' button is clicked
                    return download_dictionary(
                        request, study_obj
                    )  # Send study object to download_dictionary and receive a CSV of item data

                elif (
                    "view_all" in request.POST
                ):  # If 'Show All' or 'Show 20' button is clicked
                    if (
                        request.POST["view_all"] == "Show All"
                    ):  # If description clicked was 'Show All'
                        num_per_page = administration.objects.filter(
                            study=study_obj
                        ).count()  # Update num_per_page to the number of administration objects within study
                    elif (
                        request.POST["view_all"] == "Show 20"
                    ):  # If description clicked was 'Show 20'
    ...
    if request.method == "GET" or refresh:  # If fetching data for console rendering
        ...
        if study_name is not None:
            ...
            pass
        return render(
            request, "researcher_UI/interface.html", context
...
def rename_study(request, study_name):  # Function for study settings modal
    ...
    if request.method == "POST":  # If submitting data
        ...
        if form.is_valid():  # If form passed validation checks in forms.py
            ...
            study_obj = form.save(
                commit=False
            )  # Save object but do not commit to database just yet
            study_obj.test_period = (
                raw_test_period
                if (raw_test_period >= 1 and raw_test_period <= 28)
                else 14
            )  # Check that entered test period is within the 1-28 range. If not, set to default (14)
            ...
            if new_study_name != study_name:  # If the study name has changed
                if (
                    ...
                    or "/" in new_study_name
                ):  # Check whether the new name has already been taken by this researcher
            ...
            if raw_gift_codes:  # If gift card codes were entered
                ...
                gift_regex = re.compile(
                    r"^[a-zA-Z0-9]{4}-[a-zA-Z0-9]{6}-[a-zA-Z0-9]{4}$"
                )
                gift_type = "Amazon"
                ...
                if amount_regex:  # If monetary amount was properly registered
                    ...
                    for gift_code in gift_codes:  # For each code
                        if not payment_code.objects.filter(
                        ...
                        ).exists():  # Check that the code is unique within the database
                            ...
                            )  # Add code to the list of to-be-added codes
                        else:
                            used_codes.append(
                                gift_code
                            )  # If code was already in database, place into the list of non-unique codes
                if (
                    not used_codes
                ):  # If all the entered codes were unique, pass the unique codes check
                    all_new_codes = True

            # Error reporting. Can be confusing because of the different error combinations!
            if raw_gift_codes:  # If gift codes were entered
                if (
                    not all_new_codes or not amount_regex
                ):  # And there were non-unique codes or the monetary value was not properly entered
                    data["stat"] = "error"
                    # Flag as error
                    err_msg = []
                    if not all_new_codes:
                        ...
                        # Print error for non-unique codes and display all flagged entries
                    if not amount_regex:
                        err_msg = err_msg + [
                            'Please enter in a valid amount for "Amount per Card"'
                        ]
                        # Print error and ask for re-entry of monetary value

                    data["error_message"] = "<br>".join(err_msg)
                    # Join error messages together
                    return HttpResponse(
                    ...
                    if (
                        all_new_codes
                    ):  # If there were no errors with gift card code entry
                        payment_code.objects.bulk_create(
                            new_payment_codes
                        )  # Add gift card codes to database
                        data["stat"] = "ok"
                        # Mark as entry 'ok'
                        data["redirect_url"] = (
                            "/interface/study/" + new_study_name + "/"
                        )
                        # Set to redirect back to interface
                        return HttpResponse(
                            json.dumps(data), content_type="application/json"
                        )
            else:
            ...
            data["stat"] = "re-render"
            # Mark as entry needing re-rendering
            ...
            return render(
                request, "researcher_UI/add_study_modal.html", form_package
            )  # Reload 'Update Study' modal
    else:
...
def add_study(request):  # Function for adding studies modal
    ...
    if request.method == "POST":  # If submitting data
        ...
        if form.is_valid():  # If form passed validation checks in forms.py
            study_instance = form.save(
                commit=False
            )  # Save study object but do not commit to database just yet
            ...
            if (
                not slash_in_name and not not_unique_name
            ):  # If the researcher does not already have a study with the given name
                ...
                return HttpResponse(
                    json.dumps(data), content_type="application/json"
                )  # Display error message about non-unique study back to user
            elif slash_in_name:
                data["stat"] = "error"
                # Mark entry as 'error'
                data[
    ...
    if request.method == "POST":  # If submitting data
        ...
        if form.is_valid():  # If form passed validation checks in forms.py
            ...
            for one_study in paired_studies:  # For each study selected for grouping
                permitted = study.objects.filter(
                    researcher=researcher, name=one_study
                ).exists()  # Make sure that user is allowed to edit this study (they created it themselves)
            ...
            if all(
                True for permission in permissions
            ):  # If user was allowed to edit all the studies they selected
                data["stat"] = "ok"
                # Mark entry as 'ok'
            ...
            # Re-render form
            return render(
                request, "researcher_UI/add_paired_study_modal.html", {"form": form}
            )
    else:  # If fetching form
        form = AddPairedStudyForm(
            researcher=researcher
        )  # Pull up a blank copy of form and render
        return render(
            request, "researcher_UI/add_paired_study_modal.html", {"form": form}
        )
...
def random_url_generator(
    size=64, chars="0123456789abcdef"
):  # Function for generating a string of random characters from a set. Meant for generating unique URLs for each administration.
...
def administer_new(request, study_name):  # For creating new administrations
    ...
    if request.method == "POST":  # If submitting data
        if permitted:  # Check if user is allowed to update study
            params = dict(
                request.POST
            )  # Take submitted data and format into a dictionary
            validity = True
            data["error_message"] = ""
            raw_ids_csv = (
            ...
            )  # If a CSV of subject IDs was uploaded, hold onto it

            if (
                params["new-subject-ids"][0] == ""
                and params["autogenerate-count"][0] == ""
                and raw_ids_csv is None
            ):  # Make sure that one of the entry fields has a response in it before attempting to create administrations
                validity = False
                data[
                    "error_message"
            ...
            if raw_ids_csv:  # If a CSV of subject IDs was uploaded
                if (
                    "csv-header" in request.POST
                ):  # If CSV was marked as having a header row
                    ids_df = pd.read_csv(
                        raw_ids_csv
                    )  # Convert CSV into a pandas dataframe
                    if request.POST[
                        "subject-ids-column"
                    ]:  # If the column name was specified in form
                        subj_column = request.POST["subject-ids-column"]
                        if (
                            subj_column in ids_df.columns
                        ):  # If the specifiied column name exists within that CSV
                            ids_to_add = ids_df[
                                subj_column
                            ]  # Load the data from that column
            ...
            if (
                params["new-subject-ids"][0] != ""
            ):  # If there was text entered in the textbox field
                subject_ids = re.split(
                    "[,;\s\t\n]+", str(params["new-subject-ids"][0])
                )  # Parse string into a list of string IDs by commas, semicolons, spaces, tabs, and new lines
                ...
                subject_ids_numbers = all(
                    [x.isdigit() for x in subject_ids]
                )  # Check that all string IDs are only digits
            ...
            if (
                params["autogenerate-count"][0] != ""
            ):  # If there was text entered into the autogenerate field
                autogenerate_count = params["autogenerate-count"][0]  # Grab entry
                autogenerate_count_isdigit = (
                    autogenerate_count.isdigit()
                )  # Check that the text is only digits
            ...
            if validity:  # If the entries were valid
                new_administrations = (
                    []
                )  # Create a list for adding new administration objects
                test_period = int(study_obj.test_period)  # Note test period for study
                if raw_ids_csv:  # If a CSV was uploaded

                    subject_ids = list(
                        np.unique(ids_to_add.tolist())
                    )  # Convert pandas dataframe column into a python list

                    for sid in subject_ids:  # For each ID within the list
                        new_hash = random_url_generator()  # Generate a unique hash ID
                        old_rep = administration.objects.filter(
                            study=study_obj, subject_id=sid
                        ).count()  # Count the number of administrations previously given to this subject ID
                ...
                if (
                    params["new-subject-ids"][0] != ""
                ):  # If there was text in the new_subject_ids field
                    subject_ids = re.split(
                        "[,;\s\t\n]+", str(params["new-subject-ids"][0])
                    )  # Parse by numerous text delimiters
                ...
                if (
                    params["autogenerate-count"][0] != ""
                ):  # If there was text in the autogenerate field
                    autogenerate_count = int(
                        params["autogenerate-count"][0]
                    )  # Convert the text entry into an integer
                    ...
                    max_subject_id = administration.objects.filter(
                        ...
                        "subject_id__max"
                    ]  # Find the subject ID with the largest number within this study. For example, a study with subject IDs like '3','5',and '25' would get a '25' in this field.
                    if (
                    ...
                    ):  # For each to-be-created subject ID
                        new_hash = random_url_generator()  # Generate a unique hash ID
                        administration.objects.create(
                            study=study_obj,
                            subject_id=sid,
                ...
                # Mark entry as 'ok'
                data["redirect_url"] = (
                    "/interface/study/" + study_name + "/?sort=-created_date"
                )
                # Redirect to researcher interface with newest administrations at top of table
        ...
        return render(
            request, "researcher_UI/administer_new_modal.html", context
        )  # Render blank form with added context of username, current study name, and study group.
...
def administer_new_participant(request, username, study_name):  # used for wordful study
    ...
    researcher = User.objects.get(
        username=username
    )  # Get researcher's username. Different method because current user may not be the researcher and may not be logged in
    study_obj = study.objects.get(
        name=study_name, researcher=researcher
    )  # Find the study object associated with the researcher and study name
    ...
    completed_admins = administration.objects.filter(
        study=study_obj, completed=True
    ).count()  # Count the number of completed administrations within this study
    bypass = request.GET.get(
        "bypass", None
    )  # Check if user clicked the link to bypass study cap (studies that allow for payment will not pay participants in this case)
    ...
    )  # Check if there is a cookie stored on device for a previously completed administration
    if visitor_ip:  # If the visitor IP was successfully pulled
        prev_visitor = ip_address.objects.filter(
            ip_address=visitor_ip
        ).count()  # Check if IP address was logged previously in the database (only logged for specific studies under the langcoglab account. This is under Stanford's IRB approval)

    # if you've completed x number of payed CDIs by langcog before, or y number of CDIs generally (for the cookie)
    if (
        prev_visitor < 5 and completed < 5
    ) or request.user.is_authenticated:  # If the user if the user has not visited an excessive number of times based on IP logs and cookies or if they are logged-in (therefore a vetted researcher)
        if (
            completed_admins < subject_cap
        ):  # If the number of completed tests has not reached the subject cap
    ...
    if let_through:
        ...
        sid1 = subject_id_obscured[11:].split("827483249828")[
            0
        ]  # record id is obscured in url to avoid abuse
        ...
        if subject_id:
            ...
            if num_admins == 0:  # create first administration
                ...
                num_admins == 1
            ):  # check if this is final cdi or if user is continuing first CDI
...
def administer_new_parent(
    request, username, study_name
):  # For creating single administrations. Does not require a log-in. Participants can generate their own single-use administration if given the proper link,
    ...
    if (
        prev_visitor < 1 and completed < 2
    ) or request.user.is_authenticated:  # If the user if the user has not visited an excessive number of times based on IP logs and cookies or if they are logged-in (therefore a vetted researcher)
    ...
    if let_through:  # If marked as allowed
        ...
        redirect_url = reverse(
            "overflow", args=[username, study_name]
        )  # Generate URL for overflowed participants. May or may not have option for bypass depending on context (IP address and cookies)
    return redirect(redirect_url)  # Redirect to generated URL


def overflow(
    request, username, study_name
):  # Page for overflowed studies. For studies where subject cap has been reached or participant is noted as having excessive visits
    data = {}
    ...
    ):  # If IP address appears in logs and the user is not logged-in (cannot tell if a vetted reseacher)
        data[
            "repeat"
        ] = True  # Mark as a repeat visitor. Will not be given the option to bypass in template
    data["bypass_url"] = (
    ...
    if demo_list:
        for col in demo_list:
            if "boolean" in col or col == "born_on_due_date":
                ...
                recoded_df[col] = recoded_df[col].apply(
                    lambda x: make_str(x) if x and len(x) <= 1000 else None
                )
            elif col == "language_from":
                recoded_df[col] = recoded_df[col].apply(
                    lambda x: make_str(x) if x and len(x) <= 50 else None
                )
            elif "yob" in col:
                recoded_df[col] = (
                    recoded_df[col]
                ...
                recoded_df[col] = recoded_df[col].apply(
                    lambda x: list(
                        set([y.strip() for y in make_str(x).split("/")])
                        & set(lang_choices)
                    )
                    if x
                    else []
...
def import_data(request, study_name):
    ...
    if request.method == "POST":  # If submitting data
        ...
        if form.is_valid():
            PROJECT_ROOT = settings.BASE_DIR
            print("=========================", study_obj.instrument.form)
            print("=========================", study_obj.instrument.language)
            ...
            for index, admin_row in csv_file.iterrows():
                ...
                try:
                    new_background, created = BackgroundInfo.objects.get_or_create(
                    ...
                    for index, response_row in cdi_responses_df.iterrows():
                        ...
                        if study_obj.instrument.form == "WS":
                            ...
                            if response_row["item_type"] == "first_signs":
                                if raw_value in yes_list + no_list:
                                    item_value = (
                                        "yes" if raw_value in yes_list else "no"
                                    )
                            elif (
                                response_row["item_type"] == "phrases"
                                and raw_value in yes_list
                            ):
                        ...
                        if item_value:
                            try:
                                ...
                                error_msg = (
                                    "Error importing item '%s' for subject_id '%s'"
                                    % (sid, response_row["itemID"])
                                )
                                # break
        ...
        form = ImportDataForm(
            researcher=request.user, study=study_obj
        )  # Pull up a blank copy of form and render
        return render(request, "researcher_UI/import_data.html", {"form": form})
...
class EditAdministrationView(LoginRequiredMixin, StudyOwnerMixin, UpdateView):
    ...
    old_subject_id = (
        None  # we need to store this so we can update all repeats for this subject id
    )
