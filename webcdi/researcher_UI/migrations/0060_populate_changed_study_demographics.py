...
def set_study_demographic(apps, schema_editor):
    ...
    for obj in studies:
        ...
        if obj.instrument.name == "Canadian_English_WS2":
            ...
            obj.demographic = Demographic.objects.get(
                name="Canadian_English_Split.json"
            )
        ...
        if obj.demographic:
            print(
                f"    Saving {obj.name} with instrument {obj.instrument.name} and demographic {obj.demographic}"
            )
        obj.save()
