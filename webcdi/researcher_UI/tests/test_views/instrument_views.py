...
class AddInstrumentsTest(TestCase):
    ...
    def test_get(self):
        ...
        response = self.client.get(reverse("researcher_ui:researcher_add_instruments", kwargs={"pk": self.user.id}))
        self.assertEqual(response.status_code, 200)

    def test_post(self):
        self.client.force_login(self.user)
        payload = {
            'id_allowed_instrument_families_0': True
        }
        response = self.client.post(reverse("researcher_ui:researcher_add_instruments", kwargs={"pk": self.user.id}), payload)
