...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for curr_instrument in input_instruments:
            if not curr_instrument["form"] in settings.CAT_FORMS:
                continue
            ...
            instrument_items = apps.get_model(
            ...
            if ftype == "csv":
                contents = list(
                    unicode_csv_reader(
                        open(
                            os.path.realpath(
                                settings.BASE_DIR + "/" + curr_instrument["csv_file"]
                            ),
                            encoding="utf8",
                        )
                    )
            ...
            for row in range(1, nrows):
                ...
                if len(row_values) > 1:
                    discrimination = float(
                        row_values[col_names.index("discrimination")]
                    )
                    ...
                    upper_asymptote = float(
                        row_values[col_names.index("upper_asymptote")]
                    )
                    ...
                    cat_item, created = instrument_items.objects.update_or_create(
            ...
            cat_starting_words = apps.get_model(
                app_label="cdi_forms", model_name="CatStartingWord"
            )
            if "starting_words" in curr_instrument:
                starting_words_filename = os.path.realpath(
                    settings.BASE_DIR + "/" + curr_instrument["starting_words"]
                )
                ...
                if ftype == "csv":
                    contents = list(
                        unicode_csv_reader(
                            open(starting_words_filename, encoding="utf8")
                        )
                    )
                    col_names = contents[0]
                    nrows = len(contents)
                ...
                for row in range(1, nrows):
                    ...
                    if len(row_values) > 1:
                        ...
                        instrument_item = instrument_items.objects.get(
                            instrument=instrument_obj, definition=definition
                        )
                        ...
                        age_item, created = cat_starting_words.objects.update_or_create(
                            instrument=instrument_obj, age=age, defaults=data_dict
                        )
